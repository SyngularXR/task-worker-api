...
- `BackendClient.download_file` now accepts an optional keyword-only
  ...
  and the positional signature is unchanged, so external callers and
  subclasses are unaffected. `FakeBackendClient.download_file` mirrors the
  new keyword and raises `TaskCancelled` on a set event so the test double
  stays a faithful drop-in.
