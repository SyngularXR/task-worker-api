...
The one exception is a `.part` scratch file (below) — never a published artifact: the failing publish's own, and a dead predecessor's, which is the one leftover the SDK *can* prove no live attempt holds.
...
### A killed attempt's scratch file is reclaimed by the next one

That cleanup covers every failure the process lives through. It does not cover a SIGKILL from the container runtime, or the watchdog's `os._exit` hard exit on an in-process wedge — a routine fleet event, not a machine failure. The `.part` file then outlives the attempt that wrote it, and until v0.14.0 nothing could tell it apart from a *live* attempt's scratch file, so nothing removed it: the consumer's `rmdir` found `temp/<task_id>` non-empty forever after, leaking one staging directory and one partial (GB-scale, for a colmap-splat PLY) artifact per killed publish.

So the SDK writes each scratch file under an exclusive `flock` held for as long as that file exists, and every local publish first reclaims the scratch files in its staging dir whose lock it can take. The kernel releases a lock when — and only when — the process holding it dies, however it dies, so "lockable" means "written by an attempt that no longer exists". A running successor's file is still locked and is left alone; nothing else in the directory is even a candidate, since only the exact `.<32 hex>.part` shape is matched.

Anything it cannot *prove* is dead it leaves in place — including every scratch file on a filesystem without `flock` support, where the behaviour is exactly what it was before. An orphan an operator sweeps still beats destroying a live publish. A killed attempt that is never retried also keeps its residue, but the backend never sweeps a staging dir for a task it did not record as complete either, so nothing is waiting on it.

**If you write your own staging path,** lock the scratch file for the whole copy, not just part of it, and reclaim only what you can lock.
...
**Nothing about the on-volume layout has changed since v0.4.1.** Outputs sit directly under `temp/<task_id>/`, `output_files` carries their absolute paths, and a consumer that moves the artifacts out and then `rmdir`s `temp/<task_id>` non-recursively still finds it empty. Scratch files exist only for the duration of a copy — or, if the worker is killed mid-copy, until the next attempt of that task reclaims one, which it does before it stages anything of its own. No consumer change is needed for either.
