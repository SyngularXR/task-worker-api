...
Since the backend only sweeps a staging dir (or accepts an output manifest) for a task it recorded as **complete**, those artifacts are orphans nothing ever reaches. `Worker._run_one` logs one WARNING naming them:
