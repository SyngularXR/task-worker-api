...
async def test_worker_reports_cancel_as_fail_cooperative(
    ...
    async def handler(ctx, params):
        # The handler never explicitly checks, so nothing stops it early:
        # cancellation is cooperative, so it runs to completion and the
        # CancelGuard raises TaskCancelled on the way out of the guarded
        # block. See test_cancel_never_abandons_a_threadpool_handler for why
        # the SDK does not abort it instead.
        import asyncio
        for _ in range(20):
